...
def group_policy_positions(samples):
    ...
    for sample in samples:
        ...
        if sample["label"] == 1 and position["teacherIndex"] is None:
            position["teacherIndex"] = len(position["candidates"])
    ...
    for position_id in ordered_ids:
        ...
        if position["teacherIndex"] is None or not position["candidates"]:
            continue
...
def make_position_batch(positions, device):
    ...
    for position_index, position in enumerate(positions):
        targets[position_index] = int(position["teacherIndex"])
...
def evaluate_model(model, positions, device, batch_size):
    ...
    with torch.no_grad():
        for position_batch in chunked(positions, batch_size):
            features, mask, targets = make_position_batch(position_batch, device)
            ...
            top1_correct += int((predictions == targets).sum().detach().cpu().item())
            teacher_probabilities.extend(
                probabilities[torch.arange(probabilities.shape[0], device=device), targets].detach().cpu().tolist()
            )
...
def main():
    ...
    train_path = os.path.abspath(args.get("train", "ml/datasets/policy-train.jsonl"))
    validation_path = os.path.abspath(args.get("validation", "ml/datasets/policy-validation.jsonl"))
    json_output_path = os.path.abspath(args.get("output", "ml/models/policy-model-torch.json"))
    checkpoint_output_path = os.path.abspath(args.get("checkpoint-output", "ml/models/policy-model-torch.pt"))
    report_output_path = os.path.abspath(args.get("report", "ml/reports/policy-train-torch-report.json"))
    model_type = args.get("model", "dense")
    hidden_size = int(args.get("hidden-size", "128"))
    ...
    for epoch_index in range(epochs):
        ...
        if best_validation_top1 is None or validation_metrics["top1Accuracy"] > best_validation_top1:
            improved = True
        elif (
            validation_metrics["top1Accuracy"] == best_validation_top1 and
            (best_validation_loss is None or validation_metrics["loss"] < best_validation_loss)
        ):
            improved = True
    ...
    payload = {
        "format": "chaos-chess-policy-model-v2-torch",
        "generatedAt": datetime.now(timezone.utc).isoformat(),
        "trainingConfig": {
            "backend": "torch",
            "task": "policy_ordering",
